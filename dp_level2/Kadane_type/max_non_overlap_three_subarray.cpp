...
void solve(){
    ...
    fn(i, x)window+=arr[i];
    fn(i, n){
        if(i<x-1)pre[i] = INT_MIN;
        else if(i == x-1)pre[i] = window;
        else{
            window = window + arr[i] - arr[i - x];
            pre[i] = max(window, pre[i-1]);
        }
    }

    ll y = k;
    ll window1 = 0;
    for(ll i=n-1;i>=n-y;i--)window1 += arr[i];

    for(ll i=n-1;i>=0;i--){
        if(i>n-y)suf[i] = INT_MIN;
        else if(i == n-y)suf[i] = window1;
        else{
            window1 = window1 + arr[i] - arr[i + y];
            suf[i] = max(window1, suf[i+1]);
        }
    }

    ll sum = 0;
    fsn(i, k, 2*k)sum += arr[i];

    ll ans = 0;
    for(ll i=k;i+k<n;i++){
        ans = max(ans, sum + pre[i-1] + suf[i+k]);
        sum += arr[i+k] - arr[i];
    }
    cout<<ans<<endl;
}
