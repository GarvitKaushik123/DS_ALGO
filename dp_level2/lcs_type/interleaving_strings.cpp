...
bool rec(string s1, string s2, ll i, ll j, string s, ll idx){
    if(i == -1 and j == -1){
        if(idx == -1)return true;
        return false;
    }

    if(i>=0 and j>=0 and s1[i] == s[idx] and s2[j] == s[idx]){
        return (rec(s1, s2, i-1, j, s, idx-1)) or (rec(s1, s2, i, j-1, s, idx-1));
    }else if(i>=0 and s1[i] == s[idx]){ 
        return (rec(s1, s2, i-1, j, s, idx-1));
    }else if(j>=0 and s2[j] == s[idx]){
        return (rec(s1, s2, i, j-1, s, idx-1));
    }else{
        return false;
    }
}
