...
void solve(){
    ...
    vvl dp_help(n, vl(n, false));
    fn(g, n){
        for(ll i=0,j=g;j<n;i++,j++){
            if(g==0)dp_help[i][j] = true;
            else{
                if(str[i] == str[j] and dp_help[i+1][j-1])dp_help[i][j] = true;
            }
        }
    }
    ...
    fn(g, n){
        for(ll i=0,j=g;j<n;i++,j++){
            if(g == 0)dp[i][j] = 0;
            else if(g == 1)dp[i][j] = (str[i]==str[j]?1:0);
            else{
                string t = str.substr(i, j);
                if(dp_help[i][j])dp[i][j] = 0;
                else{
                    ll ans = INT_MAX;
                    for(ll k=i;k<=j-1;k++){
