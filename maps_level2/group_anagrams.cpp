...
void print(vector<string>& arr){for(ll i=0;i<arr.size();i++)cout<<arr[i]<<' ';}
...
void solve(){
    ...
    map<map<char, ll>, vector<string>> mp;
    fn(i, n){
        // create hashmap
       map<char, ll> temp;
        fn(j, list[i].size())temp[list[i][j]]++;
        // put in mp
        mp[temp].push_back(list[i]);
    }
    for(auto e: mp){
        vector<string> ans = e.second;
        print(ans);
        cout<<endl;
    }
}
