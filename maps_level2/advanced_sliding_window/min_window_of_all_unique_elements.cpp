...
void solve(){
    ...
    while(true){
        ...
        bool flag2 = false;
        // aquire
        while(i<str.size() and ct<dct){
            i++;
            fmap[str[i]]++;
            if(fmap[str[i]] == 1 and s.find(str[i])!=s.end())ct++;
            flag1 = true;
        }
        // release
        while(j<=i and ct==dct){
            ans = max(ans, i-j);
            j++;
            fmap[str[j]]--;
            if(fmap[str[j]] == 0 and s.find(str[i])!=s.end())ct--;
            flag2 = true;
        }
        if(flag1==false and flag2==false)break;
    }
    cout<<ans<<endl;//or you can use fmap.size() and make sure you remove elements when their value becomes zero
}
int main()
{
 fast_cin();
