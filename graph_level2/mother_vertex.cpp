...
void dfs(ll curr, vector<bool>& vis, stack<ll>& st, vector<vector<ll>>& graph){
    if(vis[curr])return;
    vis[curr] = true;
    for(auto nbr: graph[curr]){
        if(vis[nbr]==false)dfs(nbr, vis, st, graph);
    }
    st.push(curr);
}

void dfs(ll curr, vector<bool>& vis, vector<vector<ll>>& graph){
    if(vis[curr])return;
    vis[curr] = true;
    for(auto nbr: graph[curr]){
        if(vis[nbr]==false)dfs(nbr, vis, graph);
    }
}
...
void solve(){
    ...
    for(ll i=0;i<n;i++){
        if(vis[i] == false)dfs(i, vis, st, graph);
    }
