...
ll find_leader(string x, map<string, string>& mp){
    if(mp.find(x) == mp.end())mp[x]=-1;
    if(mp[x] == -1)return x;
    return mp[x] = find_leader(mp[x], mp);
}
void Union(string x, string y, map<string, string>& mp){
...
void solve(){
    ll n;cin>>n;
    vector<vector<string>> similar(n, vector<string>(2));
    ...
    fn(i, m){
        string s1, s2;cin>>s1>>s2;
        if(s1 == s2 or findPar(s1, mp) == findPar(s2, mp)){
            cout<<"YES"<<endl;
        }else{
            cout<<"NO"<<endl;
        }
