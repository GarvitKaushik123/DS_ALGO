    ...
    vector<vector<string>> accountsMerge(vector<vector<string>>& accounts) {
        ...
        for(auto account: accounts){
            ...
            for(auto s: account){
                ...
                if(etn.find(s) == etn.end())etn[s] = name;

                if(etid.find(s)==etid.end()){
                    etid[s] = cnt;cnt++;
                }
                if(account.size()>1)Union(etid[s], etid[account[1]], par);
            }
        }

        map<ll, vector<string>> lte;
        for(auto e: etn){
            string email = e.first;
            ll uid = etid[email];
            ll leader = find_leader(uid, par);
            lte[leader].pb(email);
        }
        ...
        for(auto e: lte){
            ll uid = e.first;
            vector<string> b = e.second;
            if(b.size() == 0)continue;
            sort(b.begin(), b.end());
            string name = etn[b[0]];
            b.insert(b.begin(), name);
            ans.pb(b);
        }
