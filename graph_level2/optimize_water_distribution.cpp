...
void solve(){
    ...
    for(ll i=0;i<n;i++){
        graph[i+1].pb({arr[i], 0});
        graph[0].pb({arr[i], i+1});
    }
    ...
    while(!pq.empty()){
        ...
        vis[rem.second] = true;
        ans+=rem.first;
        for(ll i=0;i<graph[rem.second].size();i++){
            ll nbr = graph[rem.second][i].second;
            if(vis[nbr] == false)pq.push({graph[rem.second][i].first, nbr});
        }
    }
    cout<<ans<<endl;
}
